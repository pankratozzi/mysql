...
select concat(substring(users.first_name, 1,1), '. ', users.last_name) as name, users.email, profiles.gender, # case when - then end as ..
if (profiles.gender is null, 'no data', profiles.gender) as cond
from users left join profiles on users.id = profiles.user_id 
where year(users.created_at) between '2010' and '2015'
order by users.id desc limit 0, 50; 

#############################################
### task 1 ### файл с примерами из урока не нашел в материалах, пересматривать видео не стал, постарался 
# применить наиболее интересные (как мне показалось) команды
USE vk_les;
